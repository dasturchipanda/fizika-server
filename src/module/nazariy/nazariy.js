  ...
  const createNewNazariy = async (req, res) => {
    try {
      ...
      const newNazariy = await createNazariy( nazariy_title,nazariy_file, nazariy_video, nazariy_test);
  ...
  const deleteNazariya = async (req, res) => {
    try {
      ...
      if (!deletedNazariy) return res.status(404).json({ error: 'Nazariy not found' });
      res.json({nazariy_id: req.params.id, message: 'Nazariy deleted successfully' });
    } catch (error) {
